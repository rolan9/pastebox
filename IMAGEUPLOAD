...
?>
<form action="index.php?key=uploader" method="POST" enctype="multipart/form-data" class="upload">
    <p>
        <span class="flex_wrapper">
            <label>Bild(er)</label>
    ...
    <p>
        <span>Du kannst zu deinem/n Bild(ern) ein Wasserzeichen hinzufügen lassen (sollte nicht größer als 400 x 200px sein).</span>
        <span class="flex_wrapper">
            <label>Wasserzeichen</label>
            <input type="file" name="watermark[]" />
        </span>
    </p>
    <p id="WraperPInputUsernameUpload" style="<?php if(!isset($_SESSION['login']) || !$_SESSION['login']){echo 'display: none;';}?>">
        <span class="flex_wrapper">
            <label>Benutzername</label>
            <input id="InputUsernameUpload" type="text" name="user" value="<?php if(isset($_SESSION['username'])){echo $_SESSION['username'];}?>" readonly/>
        </span>
    </p>
    <p>
        <span class="flex_wrapper">
